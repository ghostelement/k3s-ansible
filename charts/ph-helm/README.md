...
helm repo update
helm upgrade --install ctyk-public-health harbor/ctyk-public-health
```
...
To install the chart with the release name `ctyk-public-health`:

```bash
helm install ctyk-public-health .
```
...
To uninstall/delete the `ctyk-public-health` deployment:

```bash
helm uninstall ctyk-public-health
```
...
| `image.repository`                    | Image registry/name                                                                            | `komodorio/ctyk-public-health`       |
| `image.tag`                           | Image tag                                                                                      |                                  |
...
| `updateStrategy.type`                 | Set up update strategy for ctyk-public-health installation.                                        | `RollingUpdate`                  |             
| `extraArgs`                           | Set the arguments to be supplied to the ctyk-public-health binary                                  | `[--no-browser, --bind=0.0.0.0]` |
...
```bash
helm upgrade --install ctyk-public-health komodorio/ctyk-public-health --set dashboard.allowWriteActions=true --set service.port=9090
```
