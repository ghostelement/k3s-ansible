...
helm repo update
helm upgrade --install ygt harbor/ygt
```
...
To install the chart with the release name `ygt`:
...
To uninstall/delete the `ygt` deployment:
...
| `image.repository`                    | Image registry/name                                                                            | `komodorio/ygt`       |
| `image.tag`                           | Image tag                                                                                      |                                  |
...
| `updateStrategy.type`                 | Set up update strategy for ygt installation.                                        | `RollingUpdate`                  |             
| `extraArgs`                           | Set the arguments to be supplied to the ygt binary                                  | `[--no-browser, --bind=0.0.0.0]` |
...
```bash
helm upgrade --install ygt komodorio/ygt --set dashboard.allowWriteActions=true --set service.port=9090
```
