...
helm repo update
helm upgrade --install ctyk-chronic harbor/ctyk-chronic
```
...
To install the chart with the release name `ctyk-chronic`:

```bash
helm install ctyk-chronic .
```
...
To uninstall/delete the `ctyk-chronic` deployment:

```bash
helm uninstall ctyk-chronic
```
...
| `image.repository`                    | Image registry/name                                                                            | `komodorio/ctyk-chronic`       |
| `image.tag`                           | Image tag                                                                                      |                                  |
...
| `updateStrategy.type`                 | Set up update strategy for ctyk-chronic installation.                                        | `RollingUpdate`                  |             
| `extraArgs`                           | Set the arguments to be supplied to the ctyk-chronic binary                                  | `[--no-browser, --bind=0.0.0.0]` |
...
```bash
helm upgrade --install ctyk-chronic komodorio/ctyk-chronic --set dashboard.allowWriteActions=true --set service.port=9090
```
