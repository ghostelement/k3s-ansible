...
helm repo update
helm upgrade --install ctyk-yjk-csess harbor/ctyk-yjk-csess
```
...
To install the chart with the release name `ctyk-yjk-csess`:

```bash
helm install ctyk-yjk-csess .
```
...
To uninstall/delete the `ctyk-yjk-csess` deployment:

```bash
helm uninstall ctyk-yjk-csess
```
...
| `image.repository`                    | Image registry/name                                                                            | `komodorio/ctyk-yjk-csess`       |
| `image.tag`                           | Image tag                                                                                      |                                  |
...
| `updateStrategy.type`                 | Set up update strategy for ctyk-yjk-csess installation.                                        | `RollingUpdate`                  |             
| `extraArgs`                           | Set the arguments to be supplied to the ctyk-yjk-csess binary                                  | `[--no-browser, --bind=0.0.0.0]` |
...
```bash
helm upgrade --install ctyk-yjk-csess komodorio/ctyk-yjk-csess --set dashboard.allowWriteActions=true --set service.port=9090
```
