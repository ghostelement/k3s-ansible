...
helm repo update
helm upgrade --install ctyk-yjk-lis harbor/ctyk-yjk-lis
```
...
To install the chart with the release name `ctyk-yjk-lis`:

```bash
helm install ctyk-yjk-lis .
```
...
To uninstall/delete the `ctyk-yjk-lis` deployment:

```bash
helm uninstall ctyk-yjk-lis
```
...
| `image.repository`                    | Image registry/name                                                                            | `komodorio/ctyk-yjk-lis`       |
| `image.tag`                           | Image tag                                                                                      |                                  |
...
| `updateStrategy.type`                 | Set up update strategy for ctyk-yjk-lis installation.                                        | `RollingUpdate`                  |             
| `extraArgs`                           | Set the arguments to be supplied to the ctyk-yjk-lis binary                                  | `[--no-browser, --bind=0.0.0.0]` |
...
```bash
helm upgrade --install ctyk-yjk-lis komodorio/ctyk-yjk-lis --set dashboard.allowWriteActions=true --set service.port=9090
```
