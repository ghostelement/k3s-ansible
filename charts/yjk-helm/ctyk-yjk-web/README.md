...
helm repo update
helm upgrade --install ctyk-yjk-site harbor/ctyk-yjk-site
```
...
To install the chart with the release name `ctyk-yjk-site`:

```bash
helm install ctyk-yjk-site .
```
...
To uninstall/delete the `ctyk-yjk-site` deployment:

```bash
helm uninstall ctyk-yjk-site
```
...
| `image.repository`                    | Image registry/name                                                                            | `komodorio/ctyk-yjk-site`       |
| `image.tag`                           | Image tag                                                                                      |                                  |
...
| `updateStrategy.type`                 | Set up update strategy for ctyk-yjk-site installation.                                        | `RollingUpdate`                  |             
| `extraArgs`                           | Set the arguments to be supplied to the ctyk-yjk-site binary                                  | `[--no-browser, --bind=0.0.0.0]` |
...
```bash
helm upgrade --install ctyk-yjk-site komodorio/ctyk-yjk-site --set dashboard.allowWriteActions=true --set service.port=9090
```
