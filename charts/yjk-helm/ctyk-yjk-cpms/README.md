...
helm repo update
helm upgrade --install ctyk-yjk-zyemr harbor/ctyk-yjk-zyemr
```
...
To install the chart with the release name `ctyk-yjk-zyemr`:

```bash
helm install ctyk-yjk-zyemr .
```
...
To uninstall/delete the `ctyk-yjk-zyemr` deployment:

```bash
helm uninstall ctyk-yjk-zyemr
```
...
| `image.repository`                    | Image registry/name                                                                            | `komodorio/ctyk-yjk-zyemr`       |
| `image.tag`                           | Image tag                                                                                      |                                  |
...
| `updateStrategy.type`                 | Set up update strategy for ctyk-yjk-zyemr installation.                                        | `RollingUpdate`                  |             
| `extraArgs`                           | Set the arguments to be supplied to the ctyk-yjk-zyemr binary                                  | `[--no-browser, --bind=0.0.0.0]` |
...
```bash
helm upgrade --install ctyk-yjk-zyemr komodorio/ctyk-yjk-zyemr --set dashboard.allowWriteActions=true --set service.port=9090
```
