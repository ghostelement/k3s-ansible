...
helm repo update
helm upgrade --install ctyk-yjk-system harbor/ctyk-yjk-system
```
...
To install the chart with the release name `ctyk-yjk-system`:

```bash
helm install ctyk-yjk-system .
```
...
To uninstall/delete the `ctyk-yjk-system` deployment:

```bash
helm uninstall ctyk-yjk-system
```
...
| `image.repository`                    | Image registry/name                                                                            | `komodorio/ctyk-yjk-system`       |
| `image.tag`                           | Image tag                                                                                      |                                  |
...
| `updateStrategy.type`                 | Set up update strategy for ctyk-yjk-system installation.                                        | `RollingUpdate`                  |             
| `extraArgs`                           | Set the arguments to be supplied to the ctyk-yjk-system binary                                  | `[--no-browser, --bind=0.0.0.0]` |
...
```bash
helm upgrade --install ctyk-yjk-system komodorio/ctyk-yjk-system --set dashboard.allowWriteActions=true --set service.port=9090
```
