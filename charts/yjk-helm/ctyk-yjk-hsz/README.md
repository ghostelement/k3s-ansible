...
helm repo update
helm upgrade --install ctyk-yjk-nurse harbor/ctyk-yjk-nurse
```
...
To install the chart with the release name `ctyk-yjk-nurse`:

```bash
helm install ctyk-yjk-nurse .
```
...
To uninstall/delete the `ctyk-yjk-nurse` deployment:

```bash
helm uninstall ctyk-yjk-nurse
```
...
| `image.repository`                    | Image registry/name                                                                            | `komodorio/ctyk-yjk-nurse`       |
| `image.tag`                           | Image tag                                                                                      |                                  |
...
| `updateStrategy.type`                 | Set up update strategy for ctyk-yjk-nurse installation.                                        | `RollingUpdate`                  |             
| `extraArgs`                           | Set the arguments to be supplied to the ctyk-yjk-nurse binary                                  | `[--no-browser, --bind=0.0.0.0]` |
...
```bash
helm upgrade --install ctyk-yjk-nurse komodorio/ctyk-yjk-nurse --set dashboard.allowWriteActions=true --set service.port=9090
```
