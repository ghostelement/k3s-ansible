...
helm repo update
helm upgrade --install ctyk-yjk-cmis harbor/ctyk-yjk-cmis
```
...
To install the chart with the release name `ctyk-yjk-cmis`:

```bash
helm install ctyk-yjk-cmis .
```
...
To uninstall/delete the `ctyk-yjk-cmis` deployment:

```bash
helm uninstall ctyk-yjk-cmis
```
...
| `image.repository`                    | Image registry/name                                                                            | `komodorio/ctyk-yjk-cmis`       |
| `image.tag`                           | Image tag                                                                                      |                                  |
...
| `updateStrategy.type`                 | Set up update strategy for ctyk-yjk-cmis installation.                                        | `RollingUpdate`                  |             
| `extraArgs`                           | Set the arguments to be supplied to the ctyk-yjk-cmis binary                                  | `[--no-browser, --bind=0.0.0.0]` |
...
```bash
helm upgrade --install ctyk-yjk-cmis harbor/ctyk-yjk-cmis --set dashboard.allowWriteActions=true --set service.port=9090
```
