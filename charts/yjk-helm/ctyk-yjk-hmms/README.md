...
helm repo update
helm upgrade --install ctyk-yjk-hmms harbor/ctyk-yjk-hmms
```
...
To install the chart with the release name `ctyk-yjk-hmms`:

```bash
helm install ctyk-yjk-hmms .
```
...
To uninstall/delete the `ctyk-yjk-hmms` deployment:

```bash
helm uninstall ctyk-yjk-hmms
```
...
| `image.repository`                    | Image registry/name                                                                            | `komodorio/ctyk-yjk-hmms`       |
| `image.tag`                           | Image tag                                                                                      |                                  |
...
| `updateStrategy.type`                 | Set up update strategy for ctyk-yjk-hmms installation.                                        | `RollingUpdate`                  |             
| `extraArgs`                           | Set the arguments to be supplied to the ctyk-yjk-hmms binary                                  | `[--no-browser, --bind=0.0.0.0]` |
...
```bash
helm upgrade --install ctyk-yjk-hmms komodorio/ctyk-yjk-hmms --set dashboard.allowWriteActions=true --set service.port=9090
```
