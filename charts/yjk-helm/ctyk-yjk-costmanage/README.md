...
helm repo update
helm upgrade --install ctyk-yjk-settle harbor/ctyk-yjk-settle
```
...
To install the chart with the release name `ctyk-yjk-settle`:

```bash
helm install ctyk-yjk-settle .
```
...
To uninstall/delete the `ctyk-yjk-settle` deployment:

```bash
helm uninstall ctyk-yjk-settle
```
...
| `image.repository`                    | Image registry/name                                                                            | `komodorio/ctyk-yjk-settle`       |
| `image.tag`                           | Image tag                                                                                      |                                  |
...
| `updateStrategy.type`                 | Set up update strategy for ctyk-yjk-settle installation.                                        | `RollingUpdate`                  |             
| `extraArgs`                           | Set the arguments to be supplied to the ctyk-yjk-settle binary                                  | `[--no-browser, --bind=0.0.0.0]` |
...
```bash
helm upgrade --install ctyk-yjk-settle komodorio/ctyk-yjk-settle --set dashboard.allowWriteActions=true --set service.port=9090
```
