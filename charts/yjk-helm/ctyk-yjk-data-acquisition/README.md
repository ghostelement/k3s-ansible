...
```bash
helm repo add harbor http://k3s.harbor:8090/chartrepo/xxxx
helm repo update
helm upgrade --install ctyk-data-acquisition harbor/ctyk-data-acquisition
```
...
To install the chart with the release name `ctyk-data-acquisition`:

```bash
helm install ctyk-data-acquisition .
```
...
To uninstall/delete the `ctyk-data-acquisition` deployment:

```bash
helm uninstall ctyk-data-acquisition
```
...
| `image.repository`                    | Image registry/name                                                                            | `komodorio/ctyk-data-acquisition`       |
| `image.tag`                           | Image tag                                                                                      |                                  |
...
| `updateStrategy.type`                 | Set up update strategy for ctyk-data-acquisition installation.                                        | `RollingUpdate`                  |             
| `extraArgs`                           | Set the arguments to be supplied to the ctyk-data-acquisition binary                                  | `[--no-browser, --bind=0.0.0.0]` |
...
```bash
helm upgrade --install ctyk-data-acquisition komodorio/ctyk-data-acquisition --set dashboard.allowWriteActions=true --set service.port=9090
```
