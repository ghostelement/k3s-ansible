...
helm repo update
helm upgrade --install ctyk-yjk-tigc harbor/ctyk-yjk-tigc
```
...
To install the chart with the release name `ctyk-yjk-tigc`:

```bash
helm install ctyk-yjk-tigc .
```
...
To uninstall/delete the `ctyk-yjk-tigc` deployment:

```bash
helm uninstall ctyk-yjk-tigc
```
...
| `image.repository`                    | Image registry/name                                                                            | `komodorio/ctyk-yjk-tigc`       |
| `image.tag`                           | Image tag                                                                                      |                                  |
...
| `updateStrategy.type`                 | Set up update strategy for ctyk-yjk-tigc installation.                                        | `RollingUpdate`                  |             
| `extraArgs`                           | Set the arguments to be supplied to the ctyk-yjk-tigc binary                                  | `[--no-browser, --bind=0.0.0.0]` |
...
```bash
helm upgrade --install ctyk-yjk-tigc komodorio/ctyk-yjk-tigc --set dashboard.allowWriteActions=true --set service.port=9090
```
