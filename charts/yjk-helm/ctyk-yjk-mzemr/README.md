...
helm repo update
helm upgrade --install ctyk-yjk-mz harbor/ctyk-yjk-mz
```
...
To install the chart with the release name `ctyk-yjk-mz`:

```bash
helm install ctyk-yjk-mz .
```
...
To uninstall/delete the `ctyk-yjk-mz` deployment:

```bash
helm uninstall ctyk-yjk-mz
```
...
| `image.repository`                    | Image registry/name                                                                            | `komodorio/ctyk-yjk-mz`       |
| `image.tag`                           | Image tag                                                                                      |                                  |
...
| `updateStrategy.type`                 | Set up update strategy for ctyk-yjk-mz installation.                                        | `RollingUpdate`                  |             
| `extraArgs`                           | Set the arguments to be supplied to the ctyk-yjk-mz binary                                  | `[--no-browser, --bind=0.0.0.0]` |
...
```bash
helm upgrade --install ctyk-yjk-mz komodorio/ctyk-yjk-mz --set dashboard.allowWriteActions=true --set service.port=9090
```
