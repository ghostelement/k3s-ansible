...
helm repo update
helm upgrade --install ctyk-yjk-amc harbor/ctyk-yjk-amc
```
...
To install the chart with the release name `ctyk-yjk-amc`:

```bash
helm install ctyk-yjk-amc .
```
...
To uninstall/delete the `ctyk-yjk-amc` deployment:

```bash
helm uninstall ctyk-yjk-amc
```
...
| `image.repository`                    | Image registry/name                                                                            | `komodorio/ctyk-yjk-amc`       |
| `image.tag`                           | Image tag                                                                                      |                                  |
...
| `updateStrategy.type`                 | Set up update strategy for ctyk-yjk-amc installation.                                        | `RollingUpdate`                  |             
| `extraArgs`                           | Set the arguments to be supplied to the ctyk-yjk-amc binary                                  | `[--no-browser, --bind=0.0.0.0]` |
...
```bash
helm upgrade --install ctyk-yjk-amc komodorio/ctyk-yjk-amc --set dashboard.allowWriteActions=true --set service.port=9090
```
