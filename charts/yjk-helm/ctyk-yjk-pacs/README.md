...
helm repo update
helm upgrade --install ctyk-yjk-pacs harbor/ctyk-yjk-pacs
```
...
To install the chart with the release name `ctyk-yjk-pacs`:

```bash
helm install ctyk-yjk-pacs .
```
...
To uninstall/delete the `ctyk-yjk-pacs` deployment:

```bash
helm uninstall ctyk-yjk-pacs
```
...
| `image.repository`                    | Image registry/name                                                                            | `komodorio/ctyk-yjk-pacs`       |
| `image.tag`                           | Image tag                                                                                      |                                  |
...
| `updateStrategy.type`                 | Set up update strategy for ctyk-yjk-pacs installation.                                        | `RollingUpdate`                  |             
| `extraArgs`                           | Set the arguments to be supplied to the ctyk-yjk-pacs binary                                  | `[--no-browser, --bind=0.0.0.0]` |
...
```bash
helm upgrade --install ctyk-yjk-pacs komodorio/ctyk-yjk-pacs --set dashboard.allowWriteActions=true --set service.port=9090
```
