...
helm repo update
helm upgrade --install ctyk-yjk-dqs harbor/ctyk-yjk-dqs
```
...
To install the chart with the release name `ctyk-yjk-dqs`:

```bash
helm install ctyk-yjk-dqs .
```
...
To uninstall/delete the `ctyk-yjk-dqs` deployment:

```bash
helm uninstall ctyk-yjk-dqs
```
...
| `image.repository`                    | Image registry/name                                                                            | `komodorio/ctyk-yjk-dqs`       |
| `image.tag`                           | Image tag                                                                                      |                                  |
...
| `updateStrategy.type`                 | Set up update strategy for ctyk-yjk-dqs installation.                                        | `RollingUpdate`                  |             
| `extraArgs`                           | Set the arguments to be supplied to the ctyk-yjk-dqs binary                                  | `[--no-browser, --bind=0.0.0.0]` |
...
```bash
helm upgrade --install ctyk-yjk-dqs komodorio/ctyk-yjk-dqs --set dashboard.allowWriteActions=true --set service.port=9090
```
