...
helm repo update
helm upgrade --install ctyk-yjk-drug harbor/ctyk-yjk-drug
```
...
To install the chart with the release name `ctyk-yjk-drug`:

```bash
helm install ctyk-yjk-drug .
```
...
To uninstall/delete the `ctyk-yjk-drug` deployment:

```bash
helm uninstall ctyk-yjk-drug
```
...
| `image.repository`                    | Image registry/name                                                                            | `komodorio/ctyk-yjk-drug`       |
| `image.tag`                           | Image tag                                                                                      |                                  |
...
| `updateStrategy.type`                 | Set up update strategy for ctyk-yjk-drug installation.                                        | `RollingUpdate`                  |             
| `extraArgs`                           | Set the arguments to be supplied to the ctyk-yjk-drug binary                                  | `[--no-browser, --bind=0.0.0.0]` |
...
```bash
helm upgrade --install ctyk-yjk-drug komodorio/ctyk-yjk-drug --set dashboard.allowWriteActions=true --set service.port=9090
```
