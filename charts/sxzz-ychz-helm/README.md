...
helm repo update
helm upgrade --install igl-sxzz harbor/igl-sxzz
```
...
To install the chart with the release name `igl-sxzz`:

```bash
helm install igl-sxzz .
```
...
To uninstall/delete the `igl-sxzz` deployment:

```bash
helm uninstall igl-sxzz
```
...
| `image.repository`                    | Image registry/name                                                                            | `komodorio/igl-sxzz`       |
| `image.tag`                           | Image tag                                                                                      |                                  |
...
| `updateStrategy.type`                 | Set up update strategy for igl-sxzz installation.                                        | `RollingUpdate`                  |             
| `extraArgs`                           | Set the arguments to be supplied to the igl-sxzz binary                                  | `[--no-browser, --bind=0.0.0.0]` |
...
```bash
helm upgrade --install igl-sxzz komodorio/igl-sxzz --set dashboard.allowWriteActions=true --set service.port=9090
```
