...
helm repo update
helm upgrade --install kuboard harbor/kuboard
```
...
To install the chart with the release name `kuboard`:

```bash
helm install kuboard .
```
...
To uninstall/delete the `kuboard` deployment:

```bash
helm uninstall kuboard
```
...
| `image.repository`                    | Image registry/name                                                                            | `komodorio/kuboard`       |
| `image.tag`                           | Image tag                                                                                      |                                  |
...
| `updateStrategy.type`                 | Set up update strategy for kuboard installation.                                        | `RollingUpdate`                  |             
| `extraArgs`                           | Set the arguments to be supplied to the kuboard binary                                  | `[--no-browser, --bind=0.0.0.0]` |
...
```bash
helm upgrade --install kuboard komodorio/kuboard --set dashboard.allowWriteActions=true --set service.port=9090
```
