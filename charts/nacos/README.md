...
helm repo update
helm upgrade --install nacos harbor/nacos
```
...
| `image.repository`                    | Image registry/name                                                                            | `komodorio/nacos`       |
| `image.tag`                           | Image tag                                                                                      |                                  |
...
| `updateStrategy.type`                 | Set up update strategy for nacos installation.                                        | `RollingUpdate`                  |             
| `extraArgs`                           | Set the arguments to be supplied to the nacos binary                                  | `[--no-browser, --bind=0.0.0.0]` |
...
```bash
helm upgrade --install nacos komodorio/nacos --set dashboard.allowWriteActions=true --set service.port=9090
```
