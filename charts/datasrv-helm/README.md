...
helm repo update
helm upgrade --install datasrv harbor/datasrv
```
...
To install the chart with the release name `datasrv`:

```bash
helm install datasrv .
```
...
To uninstall/delete the `datasrv` deployment:

```bash
helm uninstall datasrv
```
...
| `image.repository`                    | Image registry/name                                                                            | `komodorio/datasrv`       |
| `image.tag`                           | Image tag                                                                                      |                                  |
...
| `updateStrategy.type`                 | Set up update strategy for datasrv installation.                                        | `RollingUpdate`                  |             
| `extraArgs`                           | Set the arguments to be supplied to the datasrv binary                                  | `[--no-browser, --bind=0.0.0.0]` |
...
```bash
helm upgrade --install datasrv komodorio/datasrv --set dashboard.allowWriteActions=true --set service.port=9090
```
